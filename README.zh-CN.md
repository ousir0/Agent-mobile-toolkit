...
```text
手机端
  -> ws://<局域网IP>:8787/v1/providers/personal/join
  -> token: 123456
...
5. 📸 截图给模型做判断、校验和回放
6. 📍 当无障碍树不完整时，退回坐标点击或滑动
7. 📤 在移动端任务开始前，先把截图、素材或文件上传到手机
8. 🔁 在 Codex、OpenClaw、Claude 之间复用同一套移动端 workflow
...
1. 先调用 `mobile_list_devices`
2. 再调用 `mobile_read_state`
3. 优先走 selector 自动化
4. 如果目标 App 无障碍树抓不全，先截图判断，再切到坐标点击或滑动
5. 如果任务依赖图片或文件，先上传到手机，再继续页面流转
